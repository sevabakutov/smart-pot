...
void init_global_ca_store_mqtt(void)
{
    esp_err_t esp_ret = ESP_FAIL;

    esp_ret = esp_tls_set_global_ca_store((const unsigned char *)cert, cert_len);
    if (esp_ret != ESP_OK) {
        ESP_LOGE("MY_CERTS", "Failed to set global CA store: %s", esp_err_to_name(esp_ret));
        return;
    }
    
    ESP_LOGI("MY_CERTS", "Global CA store set successfully!");
}
