...
async fn main() -> azure_iot_sdk::Result<()> {
    ...
    let scope_id = std::env::var("DPS_SCOPE_ID")
        .expect("Set the device provisioning service scope id in the DPS_SCOPE_ID environment variable");
    let registration_id = std::env::var("DPS_REGISTRATION_ID")
        .expect("Set the device provisioning service registration id in the DPS_REGISTRATION_ID environment variable");
    let device_key = std::env::var("DPS_DEVICE_KEY")
        .expect("Set the device provisioning service device key in the DPS_DEVICE_KEY environment variable");
